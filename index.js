...
// --- Tool Definitions ---
server.tool("search_engine", "Performs a search using TheCodeRunners search engine and returns the results.", {
...
// Basic Arithmetic
server.tool("calculate_add", "Adds two numbers.", {
    a: z.number().describe("The first number to add."),
    b: z.number().describe("The second number to add."),
}, async ({ a, b }) => ({
    content: [{ type: "text", text: String(a + b) }],
}));
server.tool("calculate_subtract", "Subtracts the second number from the first.", {
    a: z.number().describe("The number to subtract from."),
    b: z.number().describe("The number to subtract."),
}, async ({ a, b }) => ({
    content: [{ type: "text", text: String(a - b) }],
}));
server.tool("calculate_multiply", "Multiplies two numbers.", {
    a: z.number().describe("The first number to multiply."),
    b: z.number().describe("The second number to multiply."),
}, async ({ a, b }) => ({
    content: [{ type: "text", text: String(a * b) }],
}));
server.tool("calculate_divide", "Divides the first number by the second.", {
...
// Exponents & Roots
server.tool("calculate_power", "Calculates the base raised to the power of the exponent (base^exponent).", {
...
server.tool("calculate_sqrt", "Calculates the square root of a non-negative number.", {
    number: z
        .number()
        .describe("The number to find the square root of. Must be non-negative."),
}, async ({ number }) => {
    if (number < 0) {
        return {
            content: [
...
server.tool("calculate_sine", "Calculates the sine of an angle given in radians.", {
    angle_radians: z.number().describe("The angle in radians."),
}, async ({ angle_radians }) => ({
    content: [{ type: "text", text: String(Math.sin(angle_radians)) }],
}));
server.tool("calculate_cosine", "Calculates the cosine of an angle given in radians.", {
    angle_radians: z.number().describe("The angle in radians."),
}, async ({ angle_radians }) => ({
    content: [{ type: "text", text: String(Math.cos(angle_radians)) }],
}));
server.tool("calculate_tangent", "Calculates the tangent of an angle given in radians.", {
    angle_radians: z.number().describe("The angle in radians."),
}, async ({ angle_radians }) => ({
    content: [{ type: "text", text: String(Math.tan(angle_radians)) }],
}));
// Logarithms
server.tool("calculate_natural_log", "Calculates the natural logarithm (base e) of a positive number.", {
    number: z
        .number()
        .describe("The number to find the natural log of. Must be positive."),
}, async ({ number }) => {
    if (number <= 0) {
        return {
            content: [
...
server.tool("calculate_log10", "Calculates the base-10 logarithm of a positive number.", {
    number: z
        .number()
        .describe("The number to find the base-10 log of. Must be positive."),
}, async ({ number }) => {
    if (number <= 0) {
        return {
            content: [
...
// Constants
server.tool("get_pi", "Returns the value of the mathematical constant Pi (π).", {}, // No input schema needed
    async () => ({
        content: [{ type: "text", text: String(Math.PI) }],
    }));
server.tool("get_e", "Returns the value of Euler's number (e), the base of natural logarithms.", {}, // No input schema needed
    async () => ({
        content: [{ type: "text", text: String(Math.E) }],
    }));
// Factorial
server.tool("calculate_factorial", "Calculates the factorial of a non-negative integer (n!).", {
    number: z
        .number()
        .int()
        .nonnegative()
        .describe("The non-negative integer to calculate the factorial of."),
}, async ({ number }) => {
    try {
...
// Percentage
server.tool("calculate_percentage_of", "Calculates a specified percentage of a given number (e.g., 20% of 150).", {
    percentage: z
        .number()
